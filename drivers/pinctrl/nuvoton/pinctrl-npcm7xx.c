...
static int npcmgpio_get_value(struct gpio_chip *chip, unsigned int offset)
{
	...
	return gpio_bitop(bank, opGETBIT, offset, dir == GPIOF_DIR_OUT ?
			       ...
			       int value)
{
	...
	if (npcmgpio_get_direction(chip, offset) == GPIOF_DIR_OUT)
		gpio_bitop(bank, opSETBIT, offset, value ? NPCM_GP_N_DOS :
			   NPCM_GP_N_DOC);
}
...
static int npcmgpio_gpio_request(struct gpio_chip *chip, unsigned int offset)
{
	...
	return pinctrl_request_gpio(offset+chip->base);
}
...
static void npcmgpio_gpio_free(struct gpio_chip *chip, unsigned int offset)
{
	...
	pinctrl_free_gpio(offset+chip->base);
}
...
static void npcmgpio_irq_handler(struct irq_desc *desc)
{
	...
	for_each_set_bit(bit, (const void *)&sts, GPIO_PER_BANK) 
		generic_handle_irq(irq_linear_revmap(gc->irqdomain, bit));
